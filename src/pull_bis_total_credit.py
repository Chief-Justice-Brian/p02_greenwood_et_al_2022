...
primary credit source in
Greenwood-Hanson-Shleifer-Sorensen (2022); the IMF Global Debt Database
supplements it (3-year changes are spliced across the two sources).

The bulk file is a single zip with one CSV: one row per series with
metadata columns, then one column per quarter (e.g. ``1947-Q1``). We melt it
into a long/tidy format and keep ALL series; the cleaning stage selects the
relevant slice.
...
- Citation: BIS, "Long series on total credit to the non-financial
  sectors," BIS Statistics (data.bis.org).
"""
...
def pull_bis_total_credit(url=BIS_TOTAL_CREDIT_ZIP_URL):
    """Download the BIS WS_TC bulk zip and melt the wide CSV to long format."""
    ...
    # Drop empty cells (most series do not span the full 1940-present grid)
